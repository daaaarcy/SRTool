...
public class SMTQuery {
    ...
    public SMTQuery(File query, int timeout) {
        processBuilder = new ProcessBuilder("z3", "-smt2", "-file",
                query.getAbsolutePath());
        this.timeout = timeout;
    }

    /**
     * Execute the SMT query and return the result. This will null if the
    ...
    public String go() throws IOException, InterruptedException {
        ...
        try {
            ...
            thread1 = FileUtil.copy(process.getInputStream(),
                    new FileOutputStream(FILE_OUT));
            thread2 = FileUtil.copy(process.getErrorStream(),
                    new FileOutputStream(FILE_ERR));
            ...
            if (exitValue == null && process != null) {
                process.destroy();
            }
        }
